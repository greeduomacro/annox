...
//06FEB2008 Set currency to Gold
...
{
    public class GoldTokenVendorType
    {
...
{
    public class GoldVendorStone : Item
    {
        ...
        [Constructable]
        public GoldVendorStone()
            : base(3806)
        {
            Movable = false;
            Hue = 1000;
            Name = "Gold Vendor Stone";
            //06FEB2008 Set currency to Gold *** START ***
            Currency = "Gold";
            //06FEB2008 Set currency to Gold *** END   ***
        }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            {
                from.SendGump(new StaffGoldTokenGump(from, this));
            }
            else
            {
                Hued = false;
                Blessed = false;
                Bonded = false;
                from.SendGump(new GoldTokenGump(from, this));
            }
        }

        public GoldVendorStone(Serial serial)
            : base(serial)
        {
        }
...
{
    public class GoldTokenGump : Gump
    {
        private GoldVendorStone m_Stone;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID >= 1)
            {
                if (!m_Stone.EditMode)
                {
                    try
                    {
                        Type currency = GoldTokenVendorType.GetType(m_Stone.Currency);
                        ...
                        //                try
                        //                {
                        //                    Type type = GoldTokenVendorType.GetType(m_Stone.Item[(info.ButtonID - 1)].ToString());
                        //                    if (type != null)
                        //                    {
                        //                        try
                        //                        {
                        ...
                        //                                        else
                        //                                        {
                        //                                            from.SendGump(new GoldVendorStoneMobileHueGump(from, m_Stone, c, (string)m_Stone.GumpName[(info.ButtonID - 1)]));
                        //                                        }
                        //                                    }
                        //                                    if (c.Name != null)
                        ...
                        //                                    else
                        //                                    {
                        //                                        from.SendGump(new GoldVendorStoneItemHueGump(from, m_Stone, item, (string)m_Stone.GumpName[(info.ButtonID - 1)]));
                        //                                    }
                        //                                }
                ...
                {
                    GoldVendorStone.RemoveItem(m_Stone, (string)m_Stone.Item[(info.ButtonID - 1)], (string)m_Stone.GumpName[(info.ButtonID - 1)], (int)m_Stone.Price[(info.ButtonID - 1)], (int)m_Stone.ItemAmount[(info.ButtonID - 1)]);
                    from.SendMessage("You remove the item.");
                    from.SendGump(new GoldTokenGump(from, m_Stone));
                }
            }
            m_Stone.Hued = false;
            m_Stone.Blessed = false;
        ...
        {
            private Item m_Item;
            private GoldTokenGump m_Gump;

            public ItemHuePicker(Item item, GoldTokenGump gump)
                : base(0x1018)
            {
                m_Item = item;
                m_Gump = gump;
        ...
        private class CreatureHuePicker : HuePickers.HuePicker
        {
            ...
            public CreatureHuePicker(BaseCreature creature, GoldTokenGump gump)
                : base(0x1018)
            {
                m_Creature = creature;
                m_Gump = gump;
    ...
    public class GoldVendorStoneAddItemGump : Gump
    {
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 0)
            {
                from.SendGump(new StaffGoldTokenGump(from, m_Stone));
            }
            if (info.ButtonID == 1)
            {
                string item = "";
                ...
                from.SendGump(new GoldVendorStoneAddItemGump(from, m_Stone));
            }
        }
    }
    public class GoldVendorStoneAddHueGump : Gump
    {
        private Mobile m_Mobile;
        private GoldVendorStone m_Stone;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1)
            {
                ...
                from.SendGump(new GoldVendorStoneAddHueGump(from, m_Stone));
            }
        }
    }
    public class GoldVendorStoneRemoveHueGump : Gump
    {
        private Mobile m_Mobile;
        private GoldVendorStone m_Stone;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1)
            {
                ...
                for (int i = 0; i < m_RemoveList.Length; i++)
                    ...
                    {
                        GoldVendorStone.RemoveHue(m_Stone, m_RemoveList[i].Hue, m_RemoveList[i].HuePrice);

                        removed += 1;
                    }
                from.SendMessage("{0} hues have been removed.", removed);
                from.SendGump(new GoldVendorStoneRemoveHueGump(from, m_Stone));
            }
        }
    }
    public class GoldVendorStoneItemHueGump : Gump
    {
        private Mobile m_Mobile;
        private GoldVendorStone m_Stone;
        private Item m_Item;
        ...
        public GoldVendorStoneItemHueGump(Mobile mobile, GoldVendorStone stone, Item item, string gumpname)
            : base(25, 50)
        {
            Closable = false;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1)
            {
                ...
                if (switches.Length > 0)
                {
                    ...
                    from.SendGump(new GoldVendorStoneItemHueGump(from, m_Stone, m_Item, m_GumpName));
            }
        }
    }
    public class GoldVendorStoneMobileHueGump : Gump
    {
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1)
            {
                ...
                if (switches.Length > 0)
                {
                    ...
                    from.SendGump(new GoldVendorStoneMobileHueGump(from, m_Stone, m_Mobile, m_GumpName));
            }
        }
    }
    public class StaffGoldTokenGump : Gump
    {
        private GoldVendorStone m_Stone;
