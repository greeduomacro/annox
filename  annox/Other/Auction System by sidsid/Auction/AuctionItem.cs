...
namespace Arya.Auction
{
	...
	public class AuctionItem
	{
			...
			public static ItemInfo Deserialize( GenericReader reader, int version )
			{
				...
				switch ( version )
				{
					...
					case 0:
						...
						if ( item.m_Item is MobileStatuette && ( item.m_Item as MobileStatuette ).Creature == null )
						{
							item.m_Item.Delete();
							item.m_Item = null; // This will make this item invalid
						}
						break;
				}
		...
		static AuctionItem()
		{
			...
			if ( AuctionSystem.ClilocLocation != null )
			{
				clilocFolder = Path.GetDirectoryName( AuctionSystem.ClilocLocation );
		...
		public bool CanBid( Mobile m )
		{
			...
			if ( Creature )
			{
				if ( Pet != null && Pet.CanBeControlledBy( m ) )
					return true;
				else
					return false;
			}
		...
		public void End( Mobile m )
		{
			...
			else
			{
				...
				else
				{
					...
					m_Pending = true;
					m_PendingEnd = DateTime.Now + TimeSpan.FromDays( AuctionSystem.DaysForConfirmation );
		...
		private static void GiveItemTo( Mobile m, Item item )
		{
			...
			if ( !m.Backpack.TryDropItem( m, item, false ) )
			{
				m.BankBox.AddItem( item );
			}
		}
