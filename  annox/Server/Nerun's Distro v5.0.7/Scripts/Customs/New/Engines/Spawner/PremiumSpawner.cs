		...
		public void Spawn()
		{
			if ( this.SmartPRS != 0 )
			{
				...
				{ 
					...
					{
						if( m is PlayerMobile && m.AccessLevel == AccessLevel.Player || m is PlayerMobile && m.AccessLevel > AccessLevel.Player && m.Hidden == false )
						{
							mobs.Add( m );
						}
					}
					...
					else
					{
						...
						if ( this.DeSpawned != 1 ) // there is creature spawned
						{
							...
							foreach ( Mobile mDelA in mobsdel )
							{
								for ( int i = 0; i < m_Creatures.Count; i++ )
								{
							...
							foreach ( Item itemDel in itemsdel )
							{
								for ( int i = 0; i < m_Creatures.Count; i++ )
								{
