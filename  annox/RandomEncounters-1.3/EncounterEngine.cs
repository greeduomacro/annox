...
public class RandomEncounterEngine
{
    ...
    private static string               m_EncountersFile = "./Scripts/Custom/RandomEncounters-1.3/RandomEncounters.xml";
    //----------------------------------------------------------------------
...
//        }
    ...
    private static bool MaybeLoadXml()
    {
        if (!System.IO.File.Exists( m_EncountersFile )) 
        {
            Console.WriteLine( 
                "\n"+
                "#### RE: Did not find the config file at the following location:\n\n"+
                "                       \"{0}\"\n\n"+
    ...
    private static bool LoadXml()
    {
        ...
        try
        {
            ...
            if( !rtfm )
            {
                Console.WriteLine(
                    "\n"+
                    "##### RE: SYSTEM CONFIG FILE FOUND, HOWEVER IT APPEARS THAT YOU DID NOT ACTUALLY READ IT!\n\n"+
            ...
            if (facetNodes.Count==0) Console.WriteLine( "RE: There appeared to be NO FACETS defined in the configuration file!");
            //------------------------------------------------------------------
            // Iterate over facets
            //------------------------------------------------------------------
            ...
            {   
                ...
                {
                    Console.WriteLine(
                        "RE: Facet at {1} had no name. THIS IS ILLEGAL. IGNORED ENTIRE FACET!",
                ...
                if (regionNodes.Count==0) Console.WriteLine( 
                    "RE: Facet \"{0}\" at {1} had no elements. IGNORED.", 
                    facetName,
                    facetNode.Attributes[ "lineNumber" ].Value
                    );
                //--------------------------------------------------------------
                ...
                foreach ( XmlNode regionNode in regionNodes )
                {
                    ...
                    if (encounterNodes.Count==0) Console.WriteLine( 
                        "RE: {0} Region \"{1}\" at {2} had no elements. IGNORED.", 
                        regionType, 
                        regionName,
                        regionNode.Attributes[ "lineNumber" ].Value
                        );
                    ...
                    foreach ( XmlNode encounterNode in encounterNodes )
                    {
                        ...
                        if (elementNodes.Count==0) Console.WriteLine( 
                            "RE: Encounter on line {0} had no children. IGNORED.", 
        ...
        {
            Console.WriteLine("RE: Exception caught attempting to load file: " + m_EncountersFile);
    ...
    {   
        ...
        {
            Console.WriteLine( 
                "RE: Tried to add {0} \"{1}\" to {2} at {3}. THIS IS ILLEGAL. IGNORED.", 
                childNode.Name,
                pick,
                parentNode.Name,
                childNode.Attributes[ "lineNumber" ].Value
    ...
    {
        Console.WriteLine( "RE: dumping tables:" );
    ...
    public static void GenerateEncounters( string timerType )
    {
        ...
        foreach (Mobile playerMobile in playerMobiles)
        {
            ...
            if( m_Debug ) 
                Console.WriteLine( 
                    "RE: Assigning player \"{0}, map=\"{1}\" to candidate set for regionType={2}, regionName=\"{3}\" ... ", 
        ...
        {
            //Console.WriteLine("RE: No one in region for timerType:" timerType);
        ...
        if (m_Debug) Console.WriteLine(
            "RE: Generating {0} encounter checks for {1} players in {2} region...", 
            chosenMobiles.Count,
            nPlayersInRegion,
            timerType
            );
        ...
        foreach (PlayerMobile chosenMobile in chosenMobiles)
        {
            ...
            if( m_Debug ) Console.WriteLine( "RE: Picking encounter for player=\"{0}, map=\"{1}\" ... ", 
                chosenMobile.Name, 
...
//        }
        ...
        )
    {
        ...
        if( m_Debug )
        {
            Console.WriteLine( "RE: generating instances: {0} ...", encounter);
...
//            }
        ...
        )
    {
        ...
        if (typeObject == null) 
        {
            Console.WriteLine( "RE: Bad attempt to generate an element of type \"{0}\".", parentElement.PickFrom );
            return;
        }
            ...
            //--------------------------------------------------------------
        {
            ...
            Console.WriteLine( "RE: Bad attempt to contruct an element of type \"{0}\", on line {1}", parentElement.PickFrom, line );
            Console.WriteLine("{0}", e);
        ...
        else if ( created is Item )
        {
            ...
            if( parentObject!=null)
            {
                if( parentObject is Mobile )
                {
                    ...
                    //Console.WriteLine( "RE: added item to mobile: " + item );
                ...
                else if( parentObject is Container )
                {
                    ...
                    //Console.WriteLine( "RE: added item to container: " + item );
                    itemParent.AddItem( item );
                }
                else
                {
                    item.MoveToWorld( spawnPoint, map );
                    //Console.WriteLine( "RE: inserted item into world: " + item );
                }
            }
            else
            {
